...
test("runRpcIteration accepts attachment-rich terminal events larger than 1 MB", async () => {
  const cwd = mkdtempSync(join(tmpdir(), "pi-ralph-rpc-"));
  try {
...
`);
    ...
    assert.equal(result.success, true);
    assert.equal(result.lastAssistantText.length, 1_100_000);
    assert.equal(result.telemetry.lastEventType, "agent_end");
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
...
`);
    ...
    assert.ok((result.telemetry.stdoutBufferBytes ?? 0) > 8_000_000);
  } finally {
    rmSync(cwd, { recursive: true, force: true });
  }
});
